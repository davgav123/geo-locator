...
\textbf{TODO sredi tekst bolje, ovo je samo neki placeholder kao}

Проблем одређивања припадности локације на мапи држави је заправо еквивалентан проблему одређивања припадности тачке полигону, где је локација тачка, а границе државе полигон. За решавање овог проблема су испробана четири потенцијална решења. Две имплементације \textit{ray casting} алгоритма, алгоритам који сабира углове (\textit{sum angles}) као и алгоритам доступан унутар Јава библиотеке \textit{awt} \textbf{РЕФЕРЕНЦЕ}.

\textbf{Опис сваког приступа}. можда само један ray casting isprobaj :shrug: da ne komplikujes rad i da objasnjavas prugu implementaciju (koja ne radi heh).

Алгоритми су тестирани на \textit{OSM} подацима који представљају Црну Гору и Словенију. Скупови су приказани црвеном бојом на слици \ref{fig:all_map_slo_mon_example}. Оба скупа података садрже одређен број локација на мапи које не припадају државама које представљају. То их чини повољним за тестирање због тога што се добијени резултати једноставно могу проверити исцртавањем.
...
Резултати добијени применом алгоритама су приказани у табели \ref{tab:tabla_rez_poly_cont_algor}. \textbf{rez promenjeni od kad je ovo pisano, proveri} Означавају удео локација који се налазе унутар граница у односу на цео скуп, за сваки алгоритам. Добијени резултати су слични, али алгоритам са сабирањем скупова даје најбоље резултате. 
...
\textit{Awt} & 0.757 & 0.951 \\ \hline
\textit{Sum angles} & 0.775 & 0.956 \\ \hline
\end{tabular}
...
\begin{figure}[!ht]
  ...
  \caption{Локације које не припадају скуповима за редом алгоритме \textit{ray casting}, \textit{awt}, \textit{sum angles}}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=1.05\textwidth]{pictures/not_slo_mon_ray_cast_1.png}
  \caption{ray cast zoomed}
...
\begin{figure}[!ht]
  ...
  \caption{sum angles zoomed}
  \label{fig:not_slo_mon_sum_angles}
\end{figure}


Намена апликације је да одреди

Како би се успешно одредиле тачке које припадају 

Скуп чворова \textit{OSM} података је представљен координатама. 
