...
\begin{description} 
	...
	\item[\textit{withColumn},] конструише нови \textit{DataFrame} додавањем колоне на почетни -- вредности нове колоне се генеришу функцијом која је прослељена као аргумент овој трансформацији;
...
% TODO: dodaj spark sekcije za UDF i repartition
% takodje dodaj poseban pasus performanse
...
У изради апликације су коришћена два скупа података. Први скуп представља \textit{OSM} податке Европе и у њему се налазе информације о географским локацијама. Изабрани \textit{OSM} скуп података је у формату \textit{osm.pbf} (енг. \textit{OSM Protocolbuffer Binary Format}) \cite{osm_pbf_format}, који је стандард за \textit{OSM} податке. Пошто \textit{Apache Spark} нема уграђену функционалност за читање података у \textit{PBF} формату, подаци се морају пребацити у неки други формат, за који постоји једноставна интеграција са \textit{Spark}-ом. Један од тих формата је формат отвореног кода, дизајниран за брзо читање и уписивање података, \textit{Apache Parquet} \cite{apache_parquet}. Фајл \textit{ОSM PBF} формата се може изменити у \textit{parquet} коришћењем апликације \textit{osm-parquetizer} \cite{osm_parquetizer}. Ова апликација прима \textit{OSM} скуп у \textit{PBF} формату и као резултат има три фајла у \textit{parquet} формату. Резултујући фајлови представљају одвојено  \textit{OSM} чворове (са \textit{node} суфиксом), путање (са \textit{way} суфиксом) и релације (са \textit{relation} суфиксом) почетног \textit{PBF} фајла. Како су за израду \textit{geo-locator} апликације потребне само локације са описима, које садрже \textit{OSM} чворови, релације и путање се могу одбацити. Укупна величина резултујућег скупа \textit{OSM} чворова који представљају Европу је око 60 гигабајта.
...
\begin{itemize}
	...
	\item алгоритам доступан у оквиру Јава \textit{awt} библиотеке, унутар \textit{java.awt.Polygon} класе \cite{java_awt_polygon}.
\end{itemize}
