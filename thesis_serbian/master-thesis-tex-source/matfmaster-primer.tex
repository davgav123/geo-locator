...
\naslov{Обрада геопросторних података коришћењем дистрибуираних система}
...
Скала (енг. Scala) је програмски језик вишег реда заснован на функционалној и објектној парадигми. Име је добила од енглеске речи \textit{scalable} због тога што је дизајнирана тако да се развија са потребама корисника. Има широк спектар примене и може се користити за писање како једноставних скрипти, тако и у изградњи великих и комплексних система. \cite{scala_prog}

Настала је 2001. на швајцарском федералном институту за технологију у Лозани (фра. \textit{École Polytechnique Fédérale de Lausanne}) од стране Мартина Одерског. Прва официјална верзија је изашла 20. јануара 2004. године.

Данас се користи од стране великих корпорација као што су Твитер (енг. Twitter), Гугл (енг. Google) и Епл (енг. Apple). Поред тога, веома је заступљена у заједници отвореног кода у пројектима као што су Спарк (енг. Spark), Кафка (енг. Kafka), Флинк (енг. Flink) и Акка (енг. Akka).

\section{Особине Скале}
\label{sec:osоб_scala}

\subsection{Објектно оријентисан језик}
\label{subsec:scala_obj_prog}

Скала је потпуно објектно оријентисан језик што значи да је свака вредност коју дефинишемо објекат као и да је свака акција коју позовемо метод. На пример, уколико имамо два цела броја и желимо да их одузмемо, позваћемо метод назван $-$ који је дефинисан у класи која представља целе бројеве, \textit{Int}. \cite{scala_prog}

\subsection{Функционалан језик}
\label{subsec:scala_funk_prog}

Поред тога што је објектни, Скала је и функционалан језик. Функционално програмирање је засновано на два принципа. Први је тај да су функције вредности првог реда. То значи да ће функције које дефинишемо од стране компајлера бити посматране на исти начин као било који други тип, на пример целобројни или стринг. Такође, функције можемо прослеђивати другим функцијама, функције могу бити повратна вредност неке друге функције и функције можемо складиштити у променљивама.

Други је тај да функције које позивамо немају никакве бочне ефекте. Дакле, једна функција има улогу само да мапира улаз у неки излаз и ништа поред тога. То значи да кад год током извршавања програма позовемо неку фунцију са истим улазом, увек ћемо добити исту излазну вредност. Другачији назив за ову особину је референцијална транспарентост. \cite{scala_prog}

Из овога произилази да функционални језици користе имутабилне структуре података што значи су такве структуре оне за које важи да се не мењају и да уколико желимо да их променимо, морамо од постојеће направити тотално нову. \cite{scala_prog}

Међутим, Скала није чисто функционалан језик, што значи да ипак можемо дефинисати функцију која има бочне ефекте или можемо користити структуре података које се могу мењати. Наравно, поред тога, Скала омогућава писање функционалног кода без бочних ефеката и са имутабилним структурама података, па је можемо користити и на тај начин. \cite{scala_prog}

\subsection{Компатибилност}
\label{subsec:scala_komp}

Скала је конструисана тако да се компајлира у Јавин ЈВМ бајткод (енг. \textit{Java JVM bytecode}) што значи да Скала може да користи Јава класе, методе и типове. На пример, Скалин објектни целобројни тип у својој имплементацији користи примитивни еквивалент из Јаве. Поред тога, Скала може користити Јава код и обогатити га на неки начин, као на пример, додавањем неке методе у већ постојећу класу. Напоменимо и то да је време извршавања Скала програма приближно извршавању Јава програмима. \cite{scala_prog}

\subsection{Концизност}
\label{subsec:scala_konc}

Програми написани у Скали често садрже мали број линија. На пример, очекијемо да код написан у Скали садржи дупло мање линија од кода написаног у Јави. Краћи програми доводе до тога да је код лакше писати и разумети, али је и мања вероватноћа да дође до неке грешке. \cite{scala_prog}

Један од много примера како Скала смањује број линија у односу на Јаву је приказан у коду у наставку који представља начине декларисања класе у та два програмска језика.
...
Као што видимо, у Скали се  не мора декларисати посебна конструктор фукција, што доводи до смањења броја линија кода. \cite{scala_prog}
...
То доноси предности које доводе до мање грешака приликом писања кода. На пример, у статички типизираним програмима ћемо у током компилације сазнати да ли смо применили неку операцију на тип на коме та операција није дозвољена. Поред тога, статичка типизираност чини рефакторисање кода поузданијим. На пример, након промене метода ћемо бити сигурни да се повратни тип није променио. \cite{scala_prog}

Скала није само статички типизиран језик већ је и језик који аутоматски закључује типове у току компилације. На пример, када декларишемо неку променљиву целобројног типа не морамо назначити њен тип, већ ће компајлер то урадити за нас. То значи да следеће две линије кода имају еквивалентно понашање.
...
Наравно, исто важи и за било који други Скала тип, не само за целобројни.

Скала програмер не мора експлицитно да наводи типове, али је то  често пожељно због тога што се тако осигурава да ће код заправо користити тип који му је намењен, али ће и побољшати читљивост и допринети документацији. \cite{scala_prog}
